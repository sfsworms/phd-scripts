...
get_data_tecan <- function(folder_loc = folder.loc, tecan_file_name = "tecan.xlsx") {
  ...
  # This does the data import. Note that it assumes that the data file is name tecan.xlsx. If that is not the case
  # edit the code below
  ...
  # This detect how much fluff there is in front of the data and then re-import dropping that fluff.  Works for
  # TECAN file because they have a 'Cycle Nr.'  column. It then detect the last cycle and cut the bottom off.
...
add_well_name <- function(data = tecan_data, folder_loc = folder.loc) {
  ...
  well.name <- well.name %>%
    filter(toupper(wellName) != "X") # 'X' indicate an empty well.
