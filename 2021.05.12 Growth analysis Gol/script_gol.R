...
gol.get_data_tecan <- function(tecan_file_name, folder_loc = "C:/Users/worms/Desktop/Gol_ Data for R analysis/TECAN"){
...
gol.get_data_hidex <- function(tecan_file_name, folder_loc = "C:/Users/worms/Desktop/Gol_ Data for R analysis/Hidex"){
...
for(i in 1:(file_list %>% length)){
  ...
  splitted.data <- multisplit(growth_data, c("wellID"))
...
#Get the Hidex files, run the data function on each one, get the mumax for each then export to xls
...
for(i in 1:(file_list %>% length)){
  growth_data <- gol.get_data_hidex(file_list[i])
  ...
  write.xlsx(mumax, paste("C:/Users/worms/Desktop/Gol_ Data for R analysis/mumax/hidex",file_list[i], sep="/"))
}

#Okay, and that other crap
...
for(i in 1:(file_list %>% length)){
  ...
  splitted.data <- multisplit(growth_data, c("strain"))
  ...
  for(j in 1:length(splitted.data)){
    fit <- fit_easylinear(splitted.data[[j]]$time, splitted.data[[j]]$OD)
    mumax_value <- coef(fit)[3]
    strain_value <- splitted.data[[j]]$strain[1] %>% as.character()
    new_line <- c(strain_value, mumax_value)
    mumax <- rbind(mumax, new_line)
  }
  result_file_name <- gsub(".csv"," mumax.xlsx", file_list[i])
  write.xlsx(mumax, paste("C:/Users/worms/Desktop/Gol_ Data for R analysis/mumax/other",result_file_name, sep="/"))
