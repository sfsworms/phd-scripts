...
```{r}
get__dna_sequence_number <- function(df){
  count <- df 
    unique() %>%
    length()
  return(count)
...
tapply(count_set, 
       count_set %>% mutate(lib_condition = paste(library, induction)) %>% select(lib_condition), 
       get__dna_sequence_number)
```
...
```{r 'Test correlation'}
ggplot(data = count_set %>% filter(library == "nnb") , aes(x= gen1_12_ratio, y = gen5_12_ratio)) +
  geom_point() +
  facet_grid(library~induction)
```
