...
Let's look at the subset of sequences with stops codons. `rsequences_with_stop` sequences (`r percent_with_stop`)% of the total) contains stop codons. Those are mostly low-abundance sequence, with an average of `r average_reads_with_stop`reads per sequence with stop codons but `r average_reads_no_stop`reads per sequence without a stop codon.
...
good_seq <- count_set %>% pull(peptide_seq) %>% unique() %>% length()
```

I subtract the sequences that do not start with the correct codon (*TGC*) and work only with those for the reast of this document. That is I keep `r good_seq` sequences out of `r unique_sequences`. 
.
