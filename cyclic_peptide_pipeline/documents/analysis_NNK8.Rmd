...
average_reads_after_nnk_filtering <- count_set %>% filter(grepl("^TGC([ATGC][ATGC][GT]){7}$", seq)) %>%  select(c("gen1_24", "gen5_24")) %>% rowSums() %>% mean() %>% round(digits = 0)

median_reads_after_nnk_filtering <- count_set %>% filter(grepl("^TGC([ATGC][ATGC][GT]){7}$", seq)) %>%  select(c("gen1_24", "gen5_24")) %>% rowSums() %>% median() %>% round(digits = 0)

average_reads_nnk_filtered_seq <- count_set %>% filter(!grepl("^TGC([ATGC][ATGC][GT]){7}$", seq)) %>%  select(c("gen1_24", "gen5_24")) %>% rowSums() %>% mean() %>% round(digits = 0)
