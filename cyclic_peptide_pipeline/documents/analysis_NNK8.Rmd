...
count_set <- read.csv2(file = file.path(directory, "count_set_long_nnk_induced_100ksample.csv")) 
...
total_count <- count_set %>% select(5,6) %>% sum()
...
```{r}
table_gen1 <- count_set %>% select(5) %>% summary() %>% data.frame()
table_gen5 <- count_set %>% select(6) %>% summary() %>% data.frame()
kable(table_gen1 , label = "Distribution of reads in the first generation")
```
...
```{r}
plot_init <- ggplot(count_set, aes(x = gen1_24_ratio, y = enrichment_ratio)) +
  geom_point()
