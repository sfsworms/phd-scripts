...
PLOC = "PLOC1"
PBAS = "PBAS1"
ABI_TRACES = [DATA1, DATA2, DATA3, DATA4, DATA9, DATA10, DATA11, DATA12, PLOC, PBAS]
...
class Read:
    def __init__(self, record, filename):
        ...
        self.traces = dict()
        self.extract_traces_from_abi(record)
    ...
    def extract_traces_from_abi(self, record):
        for trace in ABI_TRACES:
            self.traces[trace] = record.annotations[ABIF_RAW][trace]
    ...
    def get_sequence(self):
        return Seq(self.traces[PBAS].decode())

    def get_sequence_reverse(self):
        return self.get_sequence().reverse_complement()

    def get_chromatograms(self):
        chromatograms = {"A": list(self.traces[DATA10]),
                         "C": list(self.traces[DATA12]),
                         "G": list(self.traces[DATA9]),
                         "T": list(self.traces[DATA11])}
        return chromatograms

    def get_chromatograms_reverse(self):
        chromatograms = {"A": list(reversed(self.traces[DATA11])),
                         "C": list(reversed(self.traces[DATA9])),
                         "G": list(reversed(self.traces[DATA12])),
                         "T": list(reversed(self.traces[DATA10]))}
        return chromatograms

    def get_base_location(self):
        return list(self.traces[PLOC])
